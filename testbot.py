...
TRASHMAP_URL = "http://127.0.0.1:5500"
async def trashmap_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    kb = InlineKeyboardMarkup(
        [[InlineKeyboardButton("🗺 Открыть карту мусора", web_app=WebAppInfo(url=TRASHMAP_URL))]]
    )
    await update.message.reply_text("Карта мусора:", reply_markup=kb)
# =========================
# CONFIG
# =========================
...
# URL для WebApp карты мусора (когда разместишь сайт, вставишь сюда)
TRASHMAP_URL = "https://yourdomain.com/trashmap"  # <-- поменяешь на свой домен
...
def T(lang: str, key: str) -> str:
    RU = {
        ...
        "already_registered_meetup": "ℹ️ Вы уже записаны на эту встречу.",
        ...
        "trashmap_title": "🗺 Карта мусора",
        "trashmap_text": "Открой карту и отметь место с постоянным мусором:",
        "help": (
            "/start – Регистрация\n"
            "/menu – Главное меню\n"
            "/profile – Мой профиль\n"
    ...
    UZ = {
        ...
        "already_registered_meetup": "ℹ️ Siz allaqachon ro‘yxatdan o‘tgansiz.",
        ...
        "trashmap_title": "🗺 Axlat xaritasi",
        "trashmap_text": "Xaritani oching va doimiy axlat bo‘ladigan joyni belgilang:",
        "help": (
...
async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message:
        ...
        await update.callback_query.message.reply_text(text, reply_markup=get_main_keyboard(lang))
    return ConversationHandler.END


# =========================
# REGISTRATION (ФОТО УБРАЛИ)
# =========================
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if row:
        ...
        context.user_data["language"] = lang
        await update.message.reply_text(T(lang, "already_registered"), reply_markup=get_main_keyboard(lang))
        return ConversationHandler.END
...
async def reg_study(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    lang = gl(update, context)
    if lang == "ru":
        buttons = [[InlineKeyboardButton(r, callback_data=f"reg_region_{r}")] for r in REGIONS_RU]
    else:
        buttons = [[InlineKeyboardButton(r, callback_data=f"reg_region_{r}")] for r in REGIONS_UZ]
    await update.message.reply_text(T(lang, "ask_region"), reply_markup=InlineKeyboardMarkup(buttons))
    return REG_REGION
...
async def reg_region(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if region in ("г. Ташкент", "Toshkent shahri"):
        if lang == "ru":
            buttons = [[InlineKeyboardButton(d, callback_data=f"reg_dist_{d}")] for d in TASHKENT_DISTRICTS_RU]
        else:
            buttons = [[InlineKeyboardButton(d, callback_data=f"reg_dist_{d}")] for d in TASHKENT_DISTRICTS_UZ]
        await q.message.reply_text(T(lang, "ask_district_tash"), reply_markup=InlineKeyboardMarkup(buttons))
        return REG_DISTRICT
    else:
...
async def reg_district(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await target.reply_text(T(lang, "consent_text"), reply_markup=kb)
    return REG_CONSENT
...
async def reg_consent(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn.execute(
        ...
        (
            ...
            None,  # фото больше не спрашиваем
            lang,
        ),
    )
    conn.commit()
    ...
    await q.message.reply_text(T(lang, "reg_done"), reply_markup=get_main_keyboard(lang))
    return ConversationHandler.END
...
async def profile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if row["photo_file_id"]:
        await update.message.reply_photo(photo=row["photo_file_id"], caption=text, reply_markup=get_main_keyboard(lang))
    else:
        await update.message.reply_text(text, reply_markup=get_main_keyboard(lang))
...
async def balance(update: Update, context: ContextTypes.DEFAULT_TYPE):
    lang = gl(update, context)
    conn = db()
    row = conn.execute("SELECT coins FROM users WHERE telegram_id=?", (update.effective_user.id,)).fetchone()
    conn.close()
    if not row:
        await update.message.reply_text(T(lang, "profile_unreg"))
        return
    await update.message.reply_text(T(lang, "balance").format(coins=row["coins"]), reply_markup=get_main_keyboard(lang))


async def about_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    lang = gl(update, context)
    await update.message.reply_text(T(lang, "about_text"), parse_mode="HTML", reply_markup=get_main_keyboard(lang))
...
async def trashmap_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    T(lang, "trashmap_title"),
                    web_app=WebAppInfo(url=TRASHMAP_URL),
                )
            ]
        ]
    )
    await update.message.reply_text(T(lang, "trashmap_text"), reply_markup=kb)
...
async def show_tasks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(T(lang, "menu_levels"), reply_markup=kb)
    return TASK_LEVEL
...
async def list_tasks_by_level(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    rows = conn.execute(
        ...
        SELECT 
            ...
            (
                ...
                WHERE u.telegram_id=? AND s.task_id=t.id
    ...
    for r in rows:
        ...
        status_txt = ""
        if status == "pending":
            status_txt = " ⏳(на проверке)" if lang == "ru" else " ⏳(tekshiruvda)"
        elif status == "approved":
            status_txt = " ✅(принято)" if lang == "ru" else " ✅(tasdiq)"
        elif status == "rejected":
            status_txt = " ❌(отклонено)" if lang == "ru" else " ❌(rad)"

        label = f"{r['title']} (+{r['coin_reward']}⚡){status_txt}"

        if r["is_active"] == 1:
            buttons.append([InlineKeyboardButton(label, callback_data=f"submit_task_{r['id']}")])
        else:
            buttons.append([InlineKeyboardButton(f"❌ {label}", callback_data="ignore")])

    buttons.append([InlineKeyboardButton("⬅ Назад" if lang == "ru" else "⬅ Orqaga", callback_data="back_levels")])

    await q.edit_message_text(T(lang, "choose_task"), reply_markup=InlineKeyboardMarkup(buttons))
    return TASK_LIST
...
async def save_submission(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn = db()
    uid_row = conn.execute("SELECT id FROM users WHERE telegram_id=?", (user_tid,)).fetchone()
    if not uid_row:
        conn.close()
    ...
    # Если уже есть отправка по этой задаче — обновляем (пересдача)
    ...
    if existing:
        conn.execute(
            """
            UPDATE submissions
            SET proof=?, status='pending', coins_awarded=0, submitted_at=CURRENT_TIMESTAMP
            WHERE id=?
            """,
        ...
        conn.execute(
            ...
            VALUES (?, ?, ?, 'pending')
            """,
            (uid, task_id, photo_id),
        )

    conn.commit()
    conn.close()

    await update.message.reply_text(T(lang, "task_sent"), reply_markup=get_main_keyboard(lang))
    return ConversationHandler.END
...
async def submissions_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for r in rows:
        ...
        try:
            await update.message.reply_photo(photo=r["proof"], caption=caption, reply_markup=kb)
        except Exception:
...
async def submission_review(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if ok:
        ...
        conn.execute("UPDATE users SET coins=coins+? WHERE id=?", (row["coin_reward"], row["user_id"]))
        conn.execute("UPDATE submissions SET status='approved', coins_awarded=? WHERE id=?", (row["coin_reward"], sid))
        conn.commit()
        conn.close()

        await context.bot.send_message(row["telegram_id"], f"✅ «{row['title']}» — +{row['coin_reward']}⚡")
...
async def addmeetup_limit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("Введите число от 1 до 30" if lang == "ru" else "1 dan 30 gacha bo‘lgan son kiriting")
        return ADD_LIMIT
...
async def meetups_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for r in rows:
        ...
        if not max_p or cur_count < max_p:
            buttons.append([InlineKeyboardButton(T(lang, "register"), callback_data=f"mreg_{r['id']}")])

        kb = InlineKeyboardMarkup(buttons) if buttons else None

        if r["photo_file_id"]:
            try:
                await update.message.reply_photo(photo=r["photo_file_id"], caption=text, parse_mode="HTML", reply_markup=kb)
            except Exception:
...
async def meetup_register(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn = db()
    row_user = conn.execute("SELECT id FROM users WHERE telegram_id=?", (q.from_user.id,)).fetchone()
    if not row_user:
        conn.close()
    ...
    row_m = conn.execute("SELECT max_participants FROM meetups WHERE id=?", (mid,)).fetchone()
    if not row_m:
        conn.close()
        await q.message.reply_text(T(lang, "not_found"))
        return
    ...
    if max_p:
        cur_count = conn.execute("SELECT COUNT(*) FROM meetup_registrations WHERE meetup_id=?", (mid,)).fetchone()[0]
        if cur_count >= max_p:
            conn.close()
            await q.message.reply_text("❌ Места закончились." if lang == "ru" else "❌ Joylar tugagan.")
            return

    try:
        conn.execute("INSERT INTO meetup_registrations (meetup_id, user_id) VALUES (?, ?)", (mid, uid))
        conn.commit()
        await q.message.reply_text(T(lang, "registered"))
    except sqlite3.IntegrityError:
        await q.message.reply_text(T(lang, "already_registered_meetup"))
    finally:
        conn.close()
...
async def meetup_submit_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = [[InlineKeyboardButton(f"{r['title']} — {r['start_at']}", callback_data=f"msel_{r['id']}")] for r in rows]
    await update.message.reply_text(T(lang, "choose_meetup"), reply_markup=InlineKeyboardMarkup(kb))
    return MS_CHOOSE
...
async def meetup_submit_selfie(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(T(lang, "meetup_report_sent"), reply_markup=get_main_keyboard(lang))
    return ConversationHandler.END
...
async def meetup_submissions_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for r in rows:
        if r["group_photo_file_id"]:
            try:
                await update.message.reply_photo(photo=r["group_photo_file_id"], caption=f"👥 {r['title']}")
            except Exception:
                pass
        if r["selfie_file_id"]:
            try:
                await update.message.reply_photo(photo=r["selfie_file_id"], caption=f"🤳 {r['full_name']}")
            except Exception:
                pass
...
async def meetup_review(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if ok:
        ...
        await context.bot.send_message(row["telegram_id"], f"❌ Meetup «{row['title']}»")
        await q.message.reply_text(T(lang, "rejected"))
...
async def leaders(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    me = conn.execute("SELECT coins, district FROM users WHERE telegram_id=?", (update.effective_user.id,)).fetchone()
    conn.close()
    ...
    await update.message.reply_text("\n".join(lines), parse_mode="HTML", reply_markup=get_main_keyboard(lang))
...
async def volunteers_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(update.effective_user.id):
        ...
        return
    conn = db()
    rows = conn.execute("SELECT id, full_name FROM users WHERE role='volunteer' ORDER BY full_name").fetchall()
    conn.close()
    if not rows:
        await update.message.reply_text(T(lang, "volunteers_empty"))
        return
    kb = [[InlineKeyboardButton(r["full_name"], callback_data=f"vol_{r['id']}")] for r in rows]
...
# =========================
ADM_LEVEL, ADM_LIST, ADM_NEW_TITLE, ADM_NEW_DESC, ADM_NEW_REWARD, ADM_EDIT_FIELD, ADM_EDIT_VALUE = range(500, 507)
...
async def admin_pick_level(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn = db()
    rows = conn.execute("SELECT id, title, coin_reward, is_active FROM tasks WHERE level=? ORDER BY id", (level,)).fetchall()
    conn.close()

    btns = []
    for r in rows:
        prefix = "🟢" if r["is_active"] == 1 else "🔴"
        btns.append([InlineKeyboardButton(f"{prefix} {r['title']} (+{r['coin_reward']}⚡)", callback_data=f"adm_edit_{r['id']}")])
    btns.append([InlineKeyboardButton("➕", callback_data="adm_add_new")])

    await q.message.reply_text(T(lang, "choose_task"), reply_markup=InlineKeyboardMarkup(btns))
    return ADM_LIST
...
async def admin_new_reward(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("Введите число" if lang == "ru" else "Son kiriting")
        return ADM_NEW_REWARD
    ...
    conn.execute(
        ...
        VALUES (?, ?, ?, ?, 1)
        """,
        (context.user_data["new_title"], context.user_data["new_desc"], reward, context.user_data["adm_level"]),
    )
    conn.commit()
    conn.close()
...
async def admin_edit_value(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    val: object = update.message.text.strip()
...
async def rewards_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    lang = gl(update, context)
    conn = db()
    row = conn.execute("SELECT id, full_name, coins FROM users WHERE telegram_id=?", (update.effective_user.id,)).fetchone()
    conn.close()
    if not row:
        await update.message.reply_text(T(lang, "profile_unreg"))
        return

    text_lines = [T(lang, "rewards_title"), "", T(lang, "your_balance").format(coins=row["coins"]), ""]
    kb_rows: List[List[InlineKeyboardButton]] = []
    for r in REWARDS:
        name = r["name_ru"] if lang == "ru" else r["name_uz"]
        label = f"{name} — {r['cost']}⚡"
        kb_rows.append([InlineKeyboardButton(label, callback_data=f"rw_{r['id']}")])
...
async def rewards_select(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.message.reply_text(T(lang, "rewards_ok_user").format(name=name, cost=reward["cost"]))
...
async def handle_menu_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif text in ("🗺 Карта мусора", "🗺 Axlat xaritasi"):
        await trashmap_cmd(update, context)
    else:
        return
...
def build_application() -> Application:
    ...
    reg_conv = ConversationHandler(
        ...
        states={
            ...
            REG_CONSENT: [
                CallbackQueryHandler(reg_consent, pattern="^consent_(yes|no)$"),
                MessageHandler(filters.COMMAND, cancel),
            ],
        },
    ...
    # TASKS (через ConversationHandler — чтобы фото не ловилось глобально)
    task_conv = ConversationHandler(
        ...
        states={
            ...
            TASK_LIST: [
                CallbackQueryHandler(choose_task_submit, pattern="^submit_task_"),
    ...
    app.add_handler(CallbackQueryHandler(submission_review, pattern="^(sub_ok_|sub_no_)"))
    ...
    addmeetup_conv = ConversationHandler(
        ...
        states={
            ADD_TITLE: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, addmeetup_title),
                MessageHandler(filters.COMMAND, cancel),
            ],
            ADD_DESC: [
            ...
            ADD_LIMIT: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, addmeetup_limit),
                MessageHandler(filters.COMMAND, cancel),
            ],
            ADD_PHOTO: [
    ...
    meetup_submit_conv = ConversationHandler(
        ...
        states={
            MS_CHOOSE: [
                CallbackQueryHandler(meetup_submit_choose, pattern="^msel_"),
                MessageHandler(filters.COMMAND, cancel),
            ],
            MS_GROUP_PHOTO: [
                MessageHandler(filters.PHOTO, meetup_submit_group),
                MessageHandler(filters.COMMAND, cancel),
            ],
            MS_SELFIE: [
    ...
    admin_tasks_conv = ConversationHandler(
        ...
        states={
            ...
            ADM_NEW_TITLE: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, admin_new_title),
                MessageHandler(filters.COMMAND, cancel),
            ],
            ADM_NEW_DESC: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, admin_new_desc),
                MessageHandler(filters.COMMAND, cancel),
            ],
            ADM_NEW_REWARD: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, admin_new_reward),
                MessageHandler(filters.COMMAND, cancel),
            ],
            ADM_EDIT_FIELD: [
                CallbackQueryHandler(admin_edit_field, pattern="^field_(title|desc|status|reward)$"),
                MessageHandler(filters.COMMAND, cancel),
            ],
            ADM_EDIT_VALUE: [
